...
/**
 * Live Postgres integration (C8): after running both seeders the catalogs hold 9 + 10 rows
...
describe('Control & validation catalog seed (live Postgres)', () => {
  ...
  it('seeds 9 control-exception rows with correct fields', async () => {
    if (!available) return;
  ...
  it('is idempotent: re-running both seeders keeps counts at 9 + 10', async () => {
    if (!available) return;
    ...
    expect(await exRepo.List()).toHaveLength(9);
